...
if __name__ == "__main__":
    ...
    incommon = ic.convert_index(
        inoutput.common, inctree_df, target="Short Callpath")
    ...
    # index from Cnode ID to short callpath for readability
    excommon = ic.convert_index(
        exoutput.common, exctree_df, target="Short Callpath")
    ...
    ## - is not mpi calls
    significant_exnoncommon = exnoncommon.unstack(
        "Thread ID")[significant_nodes_ex & not_initialisation &
                     (~mpi_cnodes)].stack("Thread ID")
    
    # categorical out of the index
    cnodes = pd.Series(
        data=significant_exnoncommon.reset_index()['Short Callpath'],
        dtype="category")
    ...
    significant_avgextime_rank = avgextime_rank.unstack(
        "Thread ID")[significant_nodes_ex & not_initialisation &
                     (~mpi_cnodes)].stack("Thread ID")
    ...
    flops_vec = (significant_exnoncommon.PAPI_VEC_SP +
                 significant_exnoncommon.PAPI_VEC_DP) / significant_avgextime_rank
    ...
    data_to_plot = pd.concat([
        cnodes, significant_avgextime_rank, ops, flops_vec, arithint_2, arithint_3, arithint
    ],
                             axis='columns')
    data_to_plot.columns = [
        'cnode', 'time', 'ops', 'ops_vec', 'arintv2', 'arintv3','arint'
    ]
    
    data_to_plot['size'] = (
        data_to_plot.time / data_to_plot.time.max() * 100).astype(int)
    
    
    def plot_scatter(x,y,title):
        # not working due to a bug in 1.0.3
        # data_to_plot.groupby('cnode').plot.scatter(x = 'arintv2', y = 'flops_vec', s = 'size' )
        plt.figure()
        ax = plt.axes()
        ...
        data_to_plot.groupby('cnode').apply(lambda df: df.plot.scatter(
            x=x,
            y=y,
            s=df['size'],
            label=df.cnode[0],
        ...
        #plt.show()
        #plt.savefig(title.strip().replace(' ','_')+'.png')
    ...
    senc_aggregate[[col for col in senc_aggregate.columns if 'TCM' in col]].plot.barh(ax = plt.axes())
    plt.title("Cache Misses")
    fig.subplots_adjust(left=0.2)
    #plt.savefig('cache-misses.png')
    ...
    fig.subplots_adjust(left=0.2)
    #plt.savefig('aggregate-time.png')
