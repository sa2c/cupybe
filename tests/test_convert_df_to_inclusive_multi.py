...
common_excl  = output_excl['common'].pipe(mg.transpose_for_conversion).sort_index()
common_incl  = output_incl['common'].pipe(mg.transpose_for_conversion).sort_index()

noncommon_excl  = output_excl['noncommon'].pipe(mg.transpose_for_conversion).sort_index()
noncommon_incl  = output_incl['noncommon'].pipe(mg.transpose_for_conversion).sort_index()
...
common_excl = mg.select_metrics( common_excl, convertible_metrics)
common_incl_comp = mg.convert_df_to_inclusive(common_excl, calltree).sort_index()

common_incl = mg.select_metrics(common_incl, convertible_metrics)
...
noncommon_excl = mg.select_metrics( noncommon_excl, convertible_metrics)
noncommon_incl_comp = mg.convert_df_to_inclusive(noncommon_excl, calltree).sort_index()

noncommon_incl = mg.select_metrics(noncommon_incl, convertible_metrics)
